...
Module FunctionalIMP (U : UNIVERSE).
  ...
  Definition hoare_tc_triple(t:Type)
    (* Q takes in initial heap to make fresh variables easier *)
    (* For stuff like decrementing functions *)
  ...
  *)
  ...
  Lemma write_tc (p:ptr) (u:U.t) : 
    {{ fun h => lookup h p <> None }}
    write p u
    {{ fun h u' h' => h' = insert (remove h p) p u }}.
  Proof.
    mysimp ; destruct (lookup h p) ; mysimp.
  Qed.
...
Definition c7 := 
  x <- new (Nat_t 0) ; 
  (if ptr_le_gt_dec x 10 then free x else ret tt) ;;
  z <- new (Nat_t 3) ; 
  (if ptr_le_gt_dec x 10 then ret (Nat_t 42) else read x).
...
Definition p7 := 
  x <-- new_tc (Nat_t 0) ; 
  (if_tc (ptr_le_gt_dec x 10) (free_tc x) (ret_tc tt)) ;;;
  z <-- new_tc (Nat_t 0) ; 
  (if_tc (ptr_le_gt_dec x 10) (ret_tc (Nat_t 42)) (read_tc x)).
Check p7.
...
Proof.
  induction h ; repeat mysimp;
  match goal with 
    | [ |- context[ptr_le_gt_dec ?p1 ?p2] ] => destruct (ptr_le_gt_dec p1 p2)
  end ; repeat mysimp.
Qed.  
